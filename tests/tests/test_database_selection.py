...
class DatabaseSelectionTestCase(TestCase):
    ...
    def test_enqueue_to_different_databases(self):
        ...
        result_secondary = test_tasks.calculate_meaning_of_life.using(backend="secondary").enqueue()

        self.assertTrue(DBTaskResult.objects.using("default").filter(id=result_default.id).exists())
        self.assertFalse(DBTaskResult.objects.using("secondary").filter(id=result_default.id).exists())

        self.assertTrue(DBTaskResult.objects.using("secondary").filter(id=result_secondary.id).exists())
        self.assertFalse(DBTaskResult.objects.using("default").filter(id=result_secondary.id).exists())
